/*! Motion layer | Rodrigo Rodriguez site | GSAP + ScrollTrigger
    Boots AFTER window.load: the critical path (parse, paint, LCP) runs
    on a free main thread; the delight layer starts once the page is
    usable. CSS owns entrances (off main thread) and the marquee;
    site.js owns the scroll rail and --hero-shift parallax; GSAP owns
    scroll-scrubbed scenes and orchestrated staggers only.
...
(function(){
  ...
  function boot(){
    ...
    if (window.__motionBooted) return;
      ...
      document.addEventListener('keydown', function(e){
        ...
        if (e.key === SEQ[pos]){
          ...
          if (pos === SEQ.length){
            ...
            if (typeof window.__toast === 'function') window.__toast('COSMIC MODE ON');
          }
        } else {
          pos = e.key === SEQ[0] ? 1 : 0;
        }
